...
class _HomeViewState extends State<HomeView> {
  ...
  Future<void> _fetchGPSLocation() async {
    try {
      ...
      if (permission == LocationPermission.whileInUse ||
        ...
        // Fetch real AQI from Open-Meteo
        int fetchedAqi = closestStation.aqi;
        try {
          final client = HttpClient();
          final request = await client
              .getUrl(
          ...
          if (response.statusCode == 200) {
            ...
            if (decoded['current'] != null &&
                decoded['current']['us_aqi'] != null) {
              fetchedAqi = (decoded['current']['us_aqi'] as num).toInt();
            }
          }
        } catch (_) {}
        ...
        if (mounted) {
          setState(() {
            ...
            // Build dynamic station for the exact location
            _nearestStation = AqiStation(
              name: resolvedAddress.isNotEmpty
                  ? resolvedAddress.split(',').first
                  : closestStation.name,
              region: closestStation.region,
              aqi: fetchedAqi,
              position: LatLng(lat, lon),
              distance: 'Lokasi Anda Saat Ini',
              temp: closestStation.temp,
              humidity: closestStation.humidity,
            );
          });
        }
      }
  ...
  Future<void> _loadData() async {
    ...
    final int lCount;
    if (role == 'admin') {
      lCount = await FirebaseAuthService.instance.getLaporanCount();
    } else {
      lCount = await FirebaseAuthService.instance.getLaporanCount(
        userFirestoreId: userFirestoreId,
      );
    }
    final eCount = await FirebaseAuthService.instance.getEdukasiCount();

    // Get recent reports
    final List<LaporanModel> allReports;
    if (role == 'admin') {
      allReports = await FirebaseAuthService.instance.getLaporans();
    } else {
      allReports = await FirebaseAuthService.instance.getLaporans(
        userFirestoreId: userFirestoreId,
      );
